...
#   anyflow/v4 → anyflow-embed-v4 (https://anyflow-embed-v4.vercel.app)
# 成功したら done マーカーを置き、二度と実行しない（何度呼ばれても安全）。
# もう一度デプロイしたい時は $ROOT/.deploy-v4.done を消す（次のcronで上がる）。
...
say "=== v4 自動デプロイ開始 ==="
cd "$ROOT/v4" || { say "ディレクトリが無い ($ROOT/v4)"; exit 1; }
...
say "プロジェクト = ${proj:-不明}"
if [ "$proj" != "anyflow-embed-v4" ]; then
  say "プロジェクト名が anyflow-embed-v4 でないので中止（誤爆防止）"
  exit 1
fi
...
c4=$(curl -s -o /dev/null -w "%{http_code}" https://anyflow-embed-v4.vercel.app/)
say "疎通確認: v4=$c4"
if [ "$c4" = "200" ]; then
