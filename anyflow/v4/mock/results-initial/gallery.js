...
const storageKey='anyflow-initial-results-selection-v1';
...
function refresh(){
 ...
 for(const [id,card] of cards){
  ...
  card.hidden=view==='removed'?!deleted:deleted||view==='bookmarks'&&!chosen;if(!card.hidden)count++;
  ...
  const trash=card.querySelector('.archive-remove');trash.textContent=deleted?'↶':'×';trash.setAttribute('aria-label',`No.${id}を${deleted?'復元':'削除'}`);
 }
 ...
 document.querySelector('[data-view="removed"]').textContent=`削除済み ${concepts.filter(c=>removed.has(number(c.id))).length}`;
 grid.style.setProperty('--columns',Math.min(5,Math.max(3,count)));grid.setAttribute('aria-label',`初回の案・表示中${count}案`);
 empty.hidden=!!count;empty.textContent=view==='bookmarks'?'☆から気になる案を残せます。':view==='removed'?'削除した案はありません。':'「削除済み」から案を復元できます。';
}
...
for(const c of concepts){
 ...
 card.innerHTML=`<header><h2>No.${id}</h2><h3>${c.name}</h3></header><a href="./${id}/" class="archive-preview" aria-label="初回No.${id} ${c.name}を開く" title="${c.desc}"><iframe src="./${id}/?preview=1" title="No.${id}の実装プレビュー" loading="lazy" tabindex="-1" aria-hidden="true"></iframe></a><div class="archive-actions"><button class="archive-bookmark" type="button">☆</button><button class="archive-remove" type="button">×</button></div>`;
 ...
 card.querySelector('.archive-remove').onclick=()=>{const restore=removed.has(id);restore?removed.delete(id):removed.add(id);persist();refresh();status.textContent=`No.${id}を${restore?'復元しました。':'削除しました。「削除済み」から復元できます。'}`;};
}
