...
export function createAttentionStage(content,concept){
 ...
 const proof=`<section class="at-proof"><h2>${content.head}</h2><div class="at-stats">${content.stats.map((s,i)=>`<div class="at-stat" data-i="${i}"><b>${esc(s.label)}</b><span class="pf-number" data-slot-index="${i}">${esc(s.number)}</span></div>`).join('')}</div></section>`;
