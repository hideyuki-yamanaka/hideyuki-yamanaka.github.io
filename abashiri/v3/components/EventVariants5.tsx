...
/*
 * 体験セクション｜奥行きのある3Dカルーセル 2案（案34・35）
 * 2026-09-17 ヒデさん依頼（プロンプト＋参考スクショ）。
 *
 *   案34 = CSS 3D Transform ベース。カードが楕円の軌道上を回り込む
 *   案35 = Three.js / WebGL ベース。カードの面そのものを円筒状に湾曲させる
 *
 * 依頼どおり、3Dの数値はハードコードせず下の CONFIG にまとめてある。
 *
 * 共通の約束
 *   ・値は瞬間的に切り替えず lerp（damping）で補間する
 *   ・毎フレームの DOM 更新は「カードの transform だけ」。文字や画像は触らない
 *   ・PC=ドラッグ＋ホイール、スマホ=スワイプ
 *   ・prefers-reduced-motion では慣性と自動の動きを止める
 *   ・リサイズでカメラ・カードの大きさ・配置を計算し直す
 *
 * 🟡仮置き：カンプは無い。参考スクショの見え方に寄せた数値
 */
import { useEffect, useRef, useState } from "react";
import Link from "next/link";
import type { MotionValue } from "framer-motion";
import { HTitle, ITEMS, type EventItem } from "./eventParts";
...
> = {
  34: {
    ...
    note: "カードが楕円の軌道に並び、中央が正面・左右は内側を向いて奥へ回り込む。ドラッグ／スワイプ／ホイールで回せて、離すといちばん近いカードへ吸い込まれるように止まる（CSSの3D変形）",
  },
  35: {
    name: "案35 湾曲するカルーセル",
    note: "写真の面そのものが円筒状に湾曲して並ぶ。ドラッグやホイールで流れ、速く動かすと湾曲が少し強まり、止まると元に戻る（Three.js／WebGL。頂点を実際に曲げている）",
  },
};
...
function CardInfo({ it }: { it: EventItem }) {
  return (
    <Link
      href={`/spot/${it.slug}`}
      className="group/hover absolute inset-0 flex flex-col justify-end bg-gradient-to-b from-black/5 to-black/80 px-7 py-6 opacity-0 transition-opacity duration-500 ease-standard hover:opacity-100"
    >
      <div className="flex translate-y-[16px] flex-col gap-2 opacity-0 transition-all delay-75 duration-500 ease-standard group-hover/hover:translate-y-0 group-hover/hover:opacity-100">
...
function Css3dCarousel() {
  ...
  return (
    <div className="flex w-full flex-col gap-12 bg-white sm:gap-[90px]">
      ...
      <p className="px-6 text-body-14 font-extralight leading-[1.4] text-ink/40 sm:px-[147px]">
        ← 横にドラッグすると回ります
      </p>
    </div>
  );
